...
// The Window Property data accessed with string "userData".
// Used to output LED and Force Feedback to DualShock 4 controllers.
//struct PropUserData {
//  BYTE buffer[96];
//  HANDLE file;
...
bool IsDualShock4(RID_DEVICE_INFO_HID info) {
  ...
  return info.dwVendorId == sonyVendorID &&
         (info.dwProductId == ds4Gen1ProductID ||
          info.dwProductId == ds4Gen2ProductID);
}
...
bool GetStateDualShock4(HWND hWnd,
  ...
  // TODO: verify the values in these comments are correct! Took them from DualSense gihub docs!
  ...
  StringCchPrintfW(
      ...
      L"DS4 - LX:%3d LY:%3d RX:%3d RY:%3d LT:%3d RT:%3d Dpad:%1d\n", leftStickX,
  ...
  StringCchPrintfW(
      buffer, ARRAYSIZE(buffer),
      L"Battery:%3d Touch1X:%4d Touch1Y:%4d Touch2X:%4d Touch2Y:%4d\n", battery,
  ...
  if (1 & (rawData[offset + 6] >> 4))
    StringCchCatW(buffer, ARRAYSIZE(buffer), L"Share ");

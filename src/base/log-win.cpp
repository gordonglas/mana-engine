...
namespace Mana
{
	...
	static void Log(Verbosity verbosity, Channel channel, bool newline,
	...
	void LogError(Channel channel, bool newline, const wchar_t* format, ...)
	{
	...
	void LogWarning(Channel channel, bool newline, const wchar_t* format, ...)
	{
		if (!initialized) return;
	...
	void LogInfo(Channel channel, bool newline, const wchar_t* format, ...)
	{
		if (!initialized) return;
	...
	void LogVerbose(Channel channel, bool newline, const wchar_t* format, ...)
	{
		if (!initialized) return;
		...
		const wchar_t* format, va_list args)
	{
		...
		wchar_t bufStart[80];
		int written = std::swprintf(bufStart, 80, L"[%s] %s %s: ", wtime.c_str(),
	...
	static std::wstring GetTimeString()
	{
		...
		wchar_t bufTime[20];
		int written = swprintf(bufTime, 20, L"%02d:%02d:%02d.%03d",

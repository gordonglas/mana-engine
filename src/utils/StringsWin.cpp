...
int Utf16ToUtf8(char* output, size_t outputSize, const wchar_t* wide) {
  ...
  int utf8CharLen = ::WideCharToMultiByte(CP_UTF8, 0, wide, wideCharLen,
                                          nullptr, 0, nullptr, nullptr);
  if (utf8CharLen > (int)outputSize)
    return -1;

  utf8CharLen = ::WideCharToMultiByte(CP_UTF8, 0, wide, wideCharLen, output,
                                      (int)outputSize, nullptr, nullptr);
  if (utf8CharLen == 0)
    return -1;
...
int Utf8ToUtf16(wchar_t* output, size_t outputSize, const char* utf8) {
  ...
  // is buf size big enough
  int wideCharLen =
      ::MultiByteToWideChar(CP_UTF8, 0, utf8, utf8CharLen, nullptr, 0);
  if (wideCharLen > (int)outputSize)
    return -1;

  wideCharLen = ::MultiByteToWideChar(CP_UTF8, 0, utf8, utf8CharLen, output,
                                      (int)outputSize);
  if (wideCharLen == 0)
    return -1;
